...
//		assertStatementCanBeParsedAs("Table(myTableA(myColumnB in dictionary([{'myValue1'}, {'myValue2'}], {STRING15 myColumnB})), {myColumnA})", "select myColumnA from myTableA where myColumnB in ('myValue1', 'myValue2')");
	...
	public void shouldTranslateInsertInto() throws HPCCException {
		...
		assertStatementCanBeParsedAs("x := TABLE(anotherTable, {myColumnB});\nOUTPUT(TABLE(TABLE(x, {myColumnB}),{STRING50 myColumn := '', myColumnB, STRING25 myColumnB := ''}),,'~%NEWTABLE%', overwrite);\n", "insert into myTable (myColumnA) with x as (select myColumnB from anotherTable) select x.myColumnB from x");
		assertStatementCanBeParsedAs("", "insert into myTable (myColumnA, myColumnB) with x as (select myColumnB from anotherTable) select 'myValue', x.myColumnB from x");
		assertStatementCanBeParsedAs("", "insert into myTable (myColumnA, myColumnB) with x as (select myColumnB from anotherTable) select x.myColumnB, 'myValue' from x");
		assertStatementCanBeParsedAs("OUTPUT(TABLE(TABLE((TABLE(anotherTable, {myColumnB})), {myColumnB}),{STRING50 myColumn := '', myColumnB, STRING25 myColumnB := ''}),,'~%NEWTABLE%', overwrite);\n", "insert into myTable (myColumnA) select * from (select myColumnB from anotherTable)");
		assertStatementCanBeParsedAs("OUTPUT(TABLE(TABLE(anotherTable, {string50 string_myValue := 'myValue'}),{STRING50 myColumn := '', string_myValue, STRING25 myColumnB := ''}),,'~%NEWTABLE%', overwrite);\n", "insert into myTable (myColumnA) select 'myValue' from anotherTable");
